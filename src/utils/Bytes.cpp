...
void clearByteString(ByteString *bstr) {
    const auto bs = const_cast<volatile Byte *>(bstr->data());

    for (size_t i = 0; i < bstr->size(); ++i) {
        bs[i] = static_cast<Byte>(0);
    }
}
...
ByteString u32beToByteString(const uint32_t num) {
    ByteString ret;
...
ByteString u64beToByteString(const uint64_t num) {
    const ByteString left = u32beToByteString((num >> 32) & 0xFFFFFFFF);
    const ByteString right = u32beToByteString((num >> 0) & 0xFFFFFFFF);
    return left + right;
}
...
static uint64_t u64(const uint8_t n) {
    return n;
}
...
static std::string bytesToB32Chunk(const ByteString &bs) {
    ...
    for (i = 0; i < putchars; ++i) {
        ...
        if (const Byte val = (whole >> ((7 - i) * 5)) & 0x1F; val < 26) {
...
std::string toBase32(const ByteString &b32str) {
    ...
    size_t i, j;
    for (j = 0; j < b32str.size() / 5; ++j) {
        i = j * 5;
        ByteString sub(b32str, i, 5);
        std::string chk = bytesToB32Chunk(sub);
        ret.append(chk);
    }

    i = j * 5;
    if (const size_t len = b32str.size() - i; len > 0) {
        // block of size < 5 remains
        const ByteString sub(b32str, i, std::string::npos);
        const std::string chk = bytesToB32Chunk(sub);
        ret.append(chk);
    }
